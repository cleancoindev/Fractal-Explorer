...
void Shaders::SetShaders(GLuint* p, GLuint* v, GLuint* f, const char* vertShaderFile, const char* fragShaderFile)
{
	...
	*f = glCreateShader(GL_FRAGMENT_SHADER);
	...
	vs = Read(vertShaderFile);
	fs = Read(fragShaderFile);
    ...
    if (!success) {
        ...
        std::cout << "Error compiling vertex shader: " << ErrorLog << "\n";
        exit(1);
    }
	glCompileShader(*f);
    ...
    if (!success) {
        ...
        std::cout << "Error compiling fragment shader: " << ErrorLog << "\n";
        exit(1);
    }
